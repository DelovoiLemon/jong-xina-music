import Discord, { ButtonBuilder, Message, MessageComponentInteraction } from 'discord.js';
...
export default class Search extends GuildComponent {
	 ...
	 */
	private _createSearchUI(searchResults: SearchResults, page?: number): Discord.MessageCreateOptions {
		if (!page) { page = 1; }
		...
		const numbers = new Discord.ActionRowBuilder<ButtonBuilder>();
		let displayText = '';
		...
		const searchUI = new Discord.EmbedBuilder()
			.setColor(TEAL)
		...
		const navigation = new Discord.ActionRowBuilder<ButtonBuilder>()
			.addComponents(
				// Close button
				new Discord.ButtonBuilder()
					.setLabel('Done')
	...
	async search(searchString: string): Promise<void> {
		try {
				...
				try {
					...
					switch (customId.type) {
						...
						case ('select'): {
							...
							this.debug(`Setting song with {url:${song.url}} reqBy to {user.id:${interaction.authorId}}`);
			...
			const cancel = new Discord.ActionRowBuilder<ButtonBuilder>()
				.addComponents(
					new Discord.ButtonBuilder()
						.setLabel('Cancel')
						.setCustomId(JSON.stringify({ type: 'close', special: 0 }))
			...
			const loadingMsg = new Discord.EmbedBuilder().setTitle('Searching...');
			...
			if (searchResults) {
				...
				const close = new Discord.ActionRowBuilder<ButtonBuilder>()
					.addComponents(
						new Discord.ButtonBuilder()
							.setLabel('Close')
							.setCustomId(JSON.stringify({ type: 'close', special: 0 }))
				...
				this.ui.updateMsg(this.data.guildSettings.channelId, this._msgId, {
					embeds: [new Discord.EmbedBuilder().setTitle('Error while searching')],
					components: [close]
				});
			}
		}
