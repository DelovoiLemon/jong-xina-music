...
import { gh_config } from '../constants/constants';
import { InteractionInfo, MessageInfo, RemoveGuildInfo, ChildResponse } from './discord_types';
...
export default class GuildHandlerInterface {
	 ...
	 */
	constructor(guildId: string, log: Logger) {
		this.log = log;
	...
	messageHandler(message: Discord.Message): Promise<boolean> {
		return new Promise((resolve) => {
			...
			setTimeout(() => { this._events.emit(resId.toString(), { resId: resId.toString(), content: false }); }, gh_config.max_response_wait);
		});
	}
